...
object Prompt {
    ...
    fun system(): String = buildString {
        ...
        appendLine("a past run (and whatever it claims it did) never completes THIS request. Users often")
        appendLine("re-run a past task on purpose (e.g. tapping a home suggestion) — an IDENTICAL intent in")
        appendLine("history means they want it done AGAIN, fresh, right now. Never reply done on the first")
        appendLine("step by citing a previous run — judge done only from what the current screen and this")
        appendLine("run's own actions actually show.")
        appendLine()
        ...
        appendLine("When your answer names specific PLACES (restaurants, shops, addresses) the user might go")
        appendLine("to, ALSO return them structured so the OS renders tappable cards (max 3), BEST FIRST:")
        appendLine("""{"action":"done","summary":"...","places":[{"name":"<display name>",""")
        appendLine(""""detail":"<rating · reviews · distance · hours, one line>","query":"<name + area, maps-ready>",""")
        appendLine(""""why":"<top pick only: one short reason it won, from your criteria>"}]}""")
        appendLine("Only real places you actually saw on screen — never invented ones. Otherwise omit \"places\".")
        appendLine()
        ...
        appendLine()
        appendLine("LOOKUP: when the intent asks you to FIND or CHOOSE something (a place, restaurant,")
        appendLine("product), never settle for the first thing you see: scroll the results list once or")
        appendLine("twice to widen the pool, THEN choose by explicit criteria — rating AND review count,")
        appendLine("distance, open now, and the user's interests — and state WHY you chose in the summary")
        appendLine("(e.g. \"highest-rated within 7 km that's open now\"). You can only judge what the app")
        appendLine("actually shows; scrolling is how you see more of it.")
        appendLine()
